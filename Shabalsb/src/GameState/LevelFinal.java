...
public class LevelFinal extends GameState {
	...
	//Executes to switch between different map layouts
	 ...
	 //Boolean decides which map layout will be present
	    if(safeState) {
	    	tileMap.loadMap("/Maps/finalLevelSafe.map");
	     ...
	     //Conditions which change value of boolean
	    if (player.getx() > 635  && crysNum == crystals.size())
	    	safeState = false;
	    else
	    	safeState = true;
	    
	    //positions where value of crysNum changes
		...
		//Setting position of demon according to size of crystal ArrayList
		if(player.getx() > 600){
		  if(crystals.size() == 4)
		...
		//Update FireSkulls and have them shoot
		f1.checkFire(player);
		f2.checkFire(player);
		f3.checkFire(player);
		f4.checkFire(player);
		...
		//Update Player attacking enemies
